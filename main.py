...
def initGlobalData():
    ...
    globalData.pre_code_str = ""
    globalData.pre_reg_name = ""
    # 添加监视列表,trace时打印该内存的变动
    globalData.watch_addr = {0x7eae07e060: ""}
...
def hook_code(uc: unicorn.Uc, address, size, user_data):
    ...
    for inst in cs.disasm(inst_code, size):
        ...
        if globalData.has_pre and globalData.pre_reg_name:
            regindex = reg_names[globalData.pre_reg_name.upper()]
            regvalue = uc.reg_read(regindex)
            globalData.pre_code_str += "\t//%s=0x%x" % (globalData.pre_reg_name, regvalue)
            print(globalData.pre_code_str)
            globalData.pre_code_str = ""
            globalData.has_pre = False

        # 监控我关心的内存空间，如果发生变动会再打印
        if len(globalData.watch_addr) > 0:
            for i, v in globalData.watch_addr.items():
                idata = uc.mem_read(i, 0x10)
                buf = binascii.b2a_hex(idata)
                hexstr = buf.decode(encoding="utf-8")
                if globalData.watch_addr[i] == hexstr:
                    continue
                globalData.watch_addr[i] = hexstr
        ...
        globalData.pre_code_str = opstr + fenge + curreg
